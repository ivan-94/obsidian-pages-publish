...
> 文档状态：实现完成 / Quartz HAT 两轮取消问题均已修复 / 第三轮冷启动回归 HAT 待执行
>
...
| AC-QZ-04 | 固定 archive/lock SHA、受控 `npm ci`、原子激活、离线 cache hit、fallback、Repair、磁盘预算、阶段通知、取消与缓存保留测试；HAT 首轮发现取消后的 idle 被向导自动 prepare，第二轮发现 npm 退出前清理 staging 的竞态，现已增加调用链、进程组退出与临时目录清理回归测试 | 自动门禁通过；第二次 HAT 修复待冷启动复验 | UI 首装/取消/失败/Repair 的可理解性与真实网络切换 |
...
- 两轮 HAT 均在取消场景按 P0 fail-fast 停止：首轮的自动重试已由第二轮真实 GUI 复验证明修复，第二轮的 staging 清理竞态已完成代码与自动回归修复，但尚未做第三次冷启动 GUI 复验。离线、浅色/深色/窄屏/键盘、Cloudflare 首发/更新/失败恢复等场景仍未执行，因此不是候选发布验收完成。
