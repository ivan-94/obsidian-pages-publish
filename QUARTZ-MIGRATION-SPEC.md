...
| AC-QZ-06 | Unicode、中文、空格、大小写、case collision、多个 root、index/custom index、redirect 与系统路由的 Route Bridge/真实 build 测试 | 通过 | 线上 redirect 响应 |
| AC-QZ-07 | public/unlisted/private staging 与最终 HTML/JSON/XML/binary 负向检查；真实 content index smoke | 通过 | 浏览器 Search/Graph/Explorer/Backlinks/Tag/Sitemap 人工抽查 |
...
7. 单次下载上限为 64 MiB；2026-08-03 两次首次完整安装复测为 58.24 秒和 43.96 秒。自动重试次数为 0，失败后保留 active engine，由用户显式 Repair 重试。候选发布 HAT 门槛为 120 秒、总环境占用不超过 1.5 GiB、开始前至少 2 GiB 可用空间；实现会在安装前检查可用空间、激活前检查总预算，并通过稳定错误进入 UI failed/Repair 状态。UI 显示离散阶段，不承诺 `requestUrl` 无法可靠提供的字节级百分比。
8. 300 public + 30 unlisted + 30 private 的真实 Quartz 构建于 2026-08-03 最终复测为扫描 315 ms、构建 3.30 秒、heap 增量 32.1 MiB。候选门槛为构建不超过 10 秒、heap 增量不超过 256 MiB；最终门槛需在 HAT 指定机器复测。
...
- `npm test`：65 个 test files 通过、4 个按真实环境变量跳过；597 tests 通过、5 个跳过。
- 使用固定 source archive 与 Node 22.23.1 执行 `tests/quartz-engine-store-real.test.ts`：真实 `npm ci`、安全裁剪、smoke、原子激活和断网缓存复用通过；最终复验耗时 43.96 秒，前一次为 58.24 秒。
- 使用裁剪后的固定 engine 执行 `tests/quartz-real-smoke.test.ts`、`tests/quartz-site-builder-real.test.ts` 与 `tests/release-benchmark.test.ts`：真实构建、Vault sandbox、Unicode/空格/大小写/冲突 route、redirect、可见性、确定性和 360 篇基准全部通过；最终为扫描 315 ms、构建 3.30 秒、heap 增量 32.1 MiB。
