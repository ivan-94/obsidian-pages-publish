...
import {
  ...
  type LocalAssetPlan,
} from '../../src/content/local-assets';
import type { WebpDecoderBoundary } from '../../src/content/webp-decoder';
import {
...
} from '../../src/content/unsupported-syntax';
import { loadDirectoryRouteSources } from '../../src/routing/directory-route-sources';
import {
  planSiteRoutes,
  RoutePlanningError,
...
import { installDefaultMarkdownRules } from './legacy-markdown-rules';
import {
  defaultThemeCss,
  defaultThemePath,
} from './legacy-default-theme';
import {
  canonicalUrl,
...
import type {
  ...
  PreviewPage,
  SiteBuilder,
} from '../../src/site-builder/site-builder';
...
function homeSectionPages(
  ...
  snapshots: Map<
    string,
    import('../../src/publication/article-metadata').ArticleSourceSnapshot
  >,
): PreviewPage[] {
...
function indexableSitePaths(
  config: import('../../src/config/site-config').SiteConfigV1,
  routePlan: SiteRoutePlan,
  snapshots: Map<string, import('../../src/publication/article-metadata').ArticleSourceSnapshot>,
  pages: ReadonlyArray<{
    url: string;
    visibility: 'public' | 'unlisted';
  }>,
